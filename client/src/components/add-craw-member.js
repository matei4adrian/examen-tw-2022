...
const AddCrawMember = (props) => {
  const { setShowModal, shipId } = props;
  const [name, setName] = useState("");
  const [role, setRole] = useState("CAPTAIN");

  const addCrawMember = async () => {
    const bodyValues = {
      name: name,
      role: role,
    };
    await fetch(`http://localhost:8080/api/ships/${shipId}/crawMembers`, {
      method: "POST",
      headers: {
